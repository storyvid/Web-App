...
const FileList = ({
  ...
  const canDeleteFile = (file) => {
    return (
      allowDelete && (user?.role === "admin" || file.uploadedBy === user?.uid)
    );
  };
    ...
    (e) => {
      if (onUpload && e.target.files) {
        ...
        if (selectedFiles.length > 0) {
          ...
          // Reset file input after a small delay to avoid browser quirks
          setTimeout(() => {
        ...
        <>
          ...
          >
            <Tooltip title="Click to select files or drag & drop here">
              <IconButton
                sx={{
                  ...
                  borderRadius: 4,
                  cursor: "pointer",
                  pointerEvents: "none", // Let parent Box handle the click
                  "&:hover": {
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              {files.length === 0
                ? "Click to upload your first files"
      ...
      {filteredFiles.length === 0 ? (
        <Box textAlign="center" py={4}>
          <Typography variant="body2" color="text.secondary">
            {files.length === 0
              ? onUpload
                ? "Use the upload area above to add your first files"
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            startIcon={
              downloadingFiles.has(previewDialog.file?.id) ? (
